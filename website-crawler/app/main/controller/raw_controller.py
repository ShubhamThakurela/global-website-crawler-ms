...
from ..service.raw_service import crawl_by_url, get_external_urls
...
class RawCrawlUrlController(Resource):
    @api.doc(params={
        'url': {'description': 'website url', 'in': 'query', 'type': 'str'},
        'internal_page': {'description': 'Specify whether internal page need to scrap or not', 'in': 'query',
...
class RawCrawlFileController(Resource):
    @api.doc(params={
        'internal_page': {'description': 'Specify whether internal page need to scrap or not', 'in': 'query',
                          'type': 'boolean', 'default': 'true'},
        'internal_page_limit': {'description': 'Specify limit of internal page to scrap', 'in': 'query', 'type': 'int'},
        'check_history': {'description': 'Specify whether check history or not', 'in': 'query', 'type': 'boolean',
                          'default': 'false'},
    ...
    def post(self):
        ...
        try:
            ...
            file.save(file_path)
            history_path = ConstantService.histroy_path()
            output_file_name = execute(check_history, month_history, history_path, file_path, out_path, internal_page,
                                       internal_page_limit)
...
class RawDownloadController(Resource):
    @api.doc(params={
        'output_file_name': {'description': 'website crawled data output file name', 'in': 'query', 'type': 'str'}})
    def get(self):
